...
with tf.name_scope('LoadImage'):
	...
	test_image = tf.image.resize_images(test_image, (model_size, model_size),method=tf.image.ResizeMethod.AREA)
...
with tf.name_scope('summary'):
	with tf.name_scope('image_summary'):
		...
		tf.summary.image('image', tf.image.convert_image_dtype(parts, dtype=tf.uint8, saturate=True), collections=['train'])
