...
with tf.Session(config=tmp_config) as sess:
	if a.load_model is not True:
		...
		iteration = int(a.epoch*sample_size/a.batch_size)
		for step in range(iteration):
			batch_idx = random.sample(range(sample_size), a.batch_size)	
			batch_csv = csv.iloc[batch_idx,:]
			#if path col == 0
			train_imgs = np.array([cv2.imread(im) for im in batch_csv[0]])
			#print('max train imgs', np.max(train_imgs)) 
			...
			#if train_imgs.shape != (64,224,224,3):
			...
			if step % a.print_loss_freq == 0:
				...
				for i in range(step_num)
					...
					test_imgs = np.array([cv2.imread(im) for im in batch_csv[0]])
	
					print('max test imgs', np.max(test_imgs))
					test_imgs = test_imgs/255.0
					test_img = np.resize(test_img, [a.batch_size, model_size, model_size, 3])
					#if label col == 4
                    cc = test_csv[4]
