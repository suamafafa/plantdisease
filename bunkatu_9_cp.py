...
with tf.name_scope('LoadImage'):
	...
	test_label_batch, test_x_batch = tf.train.batch([test_label, test_image],batch_size=51, allow_smaller_final_batch=False)
...
test_acc = Accuracy(y_v, test_label_batch)
		...
		for step in range(iteration_num):
			...
			if step % a.print_loss_freq == 0:
				...
				for i in range(step_num):
					showy_, showlabel_, test_acc_ = sess.run([showy, showlabel, test_acc], feed_dict={am_testing:True, drop:0.0})
					print(showy_)
					print(showlabel_)
					tmp_acc += test_acc_
				...
				tmp_acc = tmp_acc/step_num 
				print("check", tmp_acc)
				#print("check_top", check_top/step_num)
				summary_writer.add_summary(tf.Summary(value=[tf.Summary.Value(tag="test_summary/test_accuracy", simple_value=tmp_acc)]), step)
