...
patch_size_v = 100
stride = 30
def extract_patch(data, size, stride, cond):
	if cond is not None: #test
		size = size//3
		stride = stride//3
	patch = tf.extract_image_patches(data, ksizes=[1, size, size, 1], strides=[1,stride, stride, 1], rates=[1, 1, 1, 1], padding='VALID')
	return patch
...
#patch = extract_patch(data,patch_size, stride, am_testing)
...
print("patch_t", patch_t)
print("patch_v", patch_v)
...
def Accuracy2(y, label_original, split_size):
	parts_split = tf.split(y, num_or_size_splits=split_size) #1枚の画像にする
...
def Accuracy4(y, label_original, split_size):
	parts_split = tf.split(y, num_or_size_splits=split_size) #1枚の画像にする
	for j in range(len(parts_split)):
		one_part = parts_split[j]
		print("**", one_part)
		regions = tf.split(one_part, num_or_size_splits=one_part.shape[0]) #同一画像内の領域群
        max_ = tf.reduce_max(one_part)
        max_g = tf.argmax(one_part, axis=1)
        print("**", max_g)
        top = tf.nn.top_k(max_g)
        top_value = max_g[top[1][0]]
        ggg = one_part[tf.cast(top_value, tf.int64)]
        print("*", ggg)
        top_value2 = tf.argmax(ggg, 1)
        ...
        for i, region in enumerate(regions):
            ...
            if t is not None:
                pred = tf.argmax(region, 1)
                pred = tf.cast(pred, tf.float32)
            aaaa = tf.reduce_max(region)
            aaaa = tf.cast(aaaa, tf.int64)


def Pred(y, label_original, split_size):
	parts_split = tf.split(y, num_or_size_splits=split_size) #1枚の画像にする
	for j in range(len(parts_split)):
		one_part = parts_split[j]
		print("**", one_part)
		...
		for i, region in enumerate(regions):
			...
			print("***", top)
			print(aaaa)
			#tt =  tf.equal(aaaa, top)
			#if tt is not None:
				#pred_ = tf.argmax(region, 1)

	return pred, top_value2
...
def Accuracy3(y, label):
	...
	for i, one_image_y in enumerate(parts_split):	
		...
		pred_fill = tf.fill(pred.shape, pred)
		label = label_split[i]
		correct_pred = tf.equal(pred_fill, answer)
		tmp_acc = tf.reduce_mean(tf.cast(correct_pred, tf.float32))

		print("predfill", pred_fill)
		if i==0:
			acc = tmp_acc
		else:
			acc = tf.add(acc, tmp_acc)
...
with tf.name_scope("accuracy"):
	...
	accuracy2 = Accuracy2(y_v, label_original, 1)
...
showy = Showy(y_v)
showlabel = Showlabel(label_original)
pred = Pred(y_v, label_original, 1)[0]
pred2 = Pred(y_v, label_original, 1)[1]
		...
		for step in range(iteration_num):
			...
			if step % a.print_loss_freq == 0:
				...
				for i in range(51):
					...
					print("pred2", sess.run(pred2, feed_dict={am_testing:True, drop:0.0}))
					print("")
