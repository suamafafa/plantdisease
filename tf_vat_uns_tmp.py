...
parser.add_argument("--csv_name", help="path for input csv file")
...
with tf.name_scope('LoadImage'):
	...
	unlabel_images = tf.train.batch([unlabel_image_dequeue], batch_size=a.batch_size, allow_smaller_final_batch=False)
	

	#test
	test_image_np = np.load("test_image.npy")
	test_label_np = np.load("test_label_int_onehot.npy")
	test_image = tf.convert_to_tensor(test_image_np, tf.float32)
	test_image = tf.image.resize_images(test_image, (model_size, model_size))
	test_label = tf.convert_to_tensor(test_label_np, tf.float32)
#--------------Model-----------------#
img_data = x_batch
label = label_batch
dropout = a.dropout
	...
	if a.load_model is not True:
		...
		#for step in range(1):
			...
			if step % a.print_loss_freq == 0:
				...
				val_step = 1
				val_acc = 0
				while val_step * a.batch_size < 186:
					...
					#step_acc = sess.run(test_ac, feed_dict=feed_dict_test)
					logits = sess.run(model(test_img_batch))
					#label = sess.run(test_label_batch)
					step_acc = sess.run(Accuracy(logits, test_label_batch))
					print(val_step, step_acc)
					val_acc += step_acc
					val_step += 1
				...
				print()
				"""
				#logits = sess.run(model(test_image))
				val_acc = sess.run(Accuracy(model(test_image), test_label))
				print("test accuracy", val_acc)
				summary_writer.add_summary(tf.Summary(value=[
					tf.Summary.Value(tag="test_summary/test_accuracy", simple_value=val_acc)
				]), step)
